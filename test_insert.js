...
async function testInsert() {
    console.log('Testing insert to inquiries table...');
    try {
        const { data, error } = await supabase.from('inquiries').insert([{
            type: '시스템 테스트',
            contact: '010-1234-5678',
            title: '테스트 문의입니다',
            content: 'RLS 정책 확인을 위한 테스트입니다.',
            status: 'new'
        }]);
        if (error) {
            console.error('Error inserting into inquiries (Full Object):', error);
            console.error('Error Message:', error.message);
            console.error('Error Code:', error.code);
        } else {
            console.log('Successfully inserted into inquiries table:', data);
        }
    } catch (err) {
