...
- [ ] **[데이터 정합성]** ID(`username`)와 닉네임(`nickname`) 매핑 구조가 100% 분리되어 있는가? (UI는 오직 닉네임만 노출)
- [ ] **[작성 시점 보존]** 게시글 및 댓글 작성 시 실시간 Join이 아닌 작성 시점의 닉네임이 기록(Snapshot)되는가?
- [ ] **[닉네임 수정 권한]** 개인회원의 닉네임 수정(1일 1회) 기능이 프로필 테이블에만 정확히 반영되는가?
- [ ] **[ID 마스킹]** 검색 결과 및 이력서 목록에서 ID가 아닌 닉네임 혹은 마스킹 처리된 이름이 노출되는가?
