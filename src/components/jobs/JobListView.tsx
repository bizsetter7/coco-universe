...
const getPayBadgeInfo = (shop: Shop) => {
    ...
    if (!isNaN(Number(amount))) {
        amount = formatKoreanMoney(amount);
    }
            ...
            <td className="py-4 pr-4 pl-2 text-right">
                <div className="flex flex-col items-end justify-center w-full">
                    <div className="flex items-center gap-1.5 shrink-0 whitespace-nowrap">
                        ...
                        <span className={`font-black text-[15px] ${brandTheme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                            {amount}
                        </span>
                    </div>
...
const getTierBadge = (tier?: string) => {
    switch (tier) {
        case 'special': return { label: '스페셜', color: 'bg-purple-100 text-purple-600' };
        case 'premium': return { label: '프리미엄', color: 'bg-indigo-100 text-indigo-600' };
        case 'grand': return { label: '그랜드', color: 'bg-amber-100 text-amber-600' };
        case 'deluxe': return { label: '디럭스', color: 'bg-blue-100 text-blue-600' };
        default: return null;
    }
};
...
const MobileJobRow = React.memo(({
    ...
    return (
        <div
            onClick={() => onSelect(shop)}
            className={`p-4 flex justify-between items-start gap-1 border-b last:border-0 ${brandTheme === 'dark' ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}
        >
            <div className="flex-1 min-w-0 flex flex-col gap-1.5 pr-2">
                {/* Line 1: Title (광고내용) */}
                <h3 className={`text-[15px] font-bold break-words line-clamp-1 ${brandTheme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>
                ...
                {/* Line 2: Tier Badge + Region + WorkType */}
                <div className="flex items-center gap-1.5 text-[12px] flex-wrap">
                    {tierInfo && (
                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${tierInfo.color}`}>
                            {tierInfo.label}
                        </span>
                    )}
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    <span className={`text-[15px] font-black ${brandTheme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                        {amount}
                    </span>
                    {shop.options?.paySuffixes?.map((suffix, i) => (
                        <span key={i} className="px-1.5 py-0.5 bg-gray-100 text-gray-600 text-[9px] rounded font-bold border border-gray-200">
