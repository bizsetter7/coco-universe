...
export default function MessageModal({ isOpen, onClose, initialReceiver }: MessageModalProps) {
    ...
    const getTabClass = (tab: Tab) => `
        ...
        ${activeTab === tab
            ? 'text-blue-600 after:content-[""] after:absolute after:bottom-0 after:left-0 after:w-full after:h-[3px] after:bg-blue-600'
        ...
        <div className="fixed inset-0 z-[20000] flex items-center justify-center bg-black/50 backdrop-blur-sm p-0 sm:p-4">
            ...
            <div className="w-full sm:w-[95%] max-w-[850px] h-full sm:h-[650px] bg-white rounded-none sm:rounded-lg shadow-xl flex flex-col overflow-hidden relative transition-transform">
                ...
                <div className="h-[45px] sm:h-[50px] bg-gray-50 flex items-center justify-between px-0 border-b border-gray-200 shrink-0 relative">
                    ...
                    <button onClick={onClose} className="absolute right-0 top-0 h-full w-12 text-gray-400 hover:text-blue-600 transition-colors flex items-center justify-center border-l border-gray-200 bg-gray-50 z-20">
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="flex flex-col md:flex-row flex-1 overflow-hidden bg-white">
                    ...
                    <div className="w-full md:w-[260px] bg-gray-100 p-3 sm:p-5 flex flex-col gap-3 sm:gap-5 shrink-0 overflow-y-auto border-r border-gray-200">
                        ...
                        <div className="text-gray-800 font-bold text-base sm:text-lg leading-tight px-1 mb-2">
                            <span className="text-blue-600">{userName}</span>님<br />환영합니다
                        ...
                        <div className="bg-white rounded-lg p-1 shadow-sm border border-gray-200">
                            ...
                            <div onClick={() => setActiveTab('inbox')} className="flex justify-between items-center cursor-pointer hover:bg-blue-50 p-2 sm:p-3 rounded transition-colors group border-b border-gray-100 last:border-0">
                                <span className={`text-xs sm:text-sm font-bold ${activeTab === 'inbox' ? 'text-blue-600' : 'text-gray-600 group-hover:text-blue-600'}`}>받은쪽지함</span>
                                <span className="font-bold text-blue-600 text-xs sm:text-sm">{inbox.length} 건</span>
                            </div>
                            ...
                            <div onClick={() => setActiveTab('unread')} className="flex justify-between items-center cursor-pointer hover:bg-blue-50 p-2 sm:p-3 rounded transition-colors group border-b border-gray-100 last:border-0">
                                <span className={`text-xs sm:text-sm font-bold ${activeTab === 'unread' ? 'text-blue-600' : 'text-gray-600 group-hover:text-blue-600'}`}>미확인쪽지</span>
                            ...
                            <div onClick={() => setActiveTab('sent')} className="flex justify-between items-center cursor-pointer hover:bg-blue-50 p-2 sm:p-3 rounded transition-colors group">
                                <span className={`text-xs sm:text-sm font-bold ${activeTab === 'sent' ? 'text-blue-600' : 'text-gray-600 group-hover:text-blue-600'}`}>보낸쪽지</span>
                                <span className="font-bold text-blue-600 text-xs sm:text-sm">{sent.length} 건</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 p-4 sm:p-6 overflow-y-auto bg-white">
                            ...
                            <div className="h-full flex flex-col">
                                <div className="border-b border-gray-200 pb-2 mb-4 text-xs sm:text-sm flex items-center">
                                    ...
                                    <input
                                        ...
                                        placeholder="[관리자]"
                                        className="flex-1 bg-gray-50 border border-gray-200 rounded px-2 py-1 font-bold text-gray-800 focus:outline-none focus:border-blue-500 transition-colors"
                                    />
                                </div>
                                <textarea
                                    className="flex-1 border border-gray-300 rounded p-4 resize-none focus:outline-none focus:border-blue-400 text-xs sm:text-sm"
                                ...
                                <div className="mt-4 flex justify-center">
                                    <button
                                        onClick={handleSend}
                                        className="bg-blue-600 text-white px-8 py-3 rounded shadow hover:bg-blue-700 font-bold w-full sm:w-auto text-sm sm:text-base"
                                    >
                                        쪽지보내기
                                    </button>
                                </div>
                        ...
                        {selectedNote && activeTab !== 'write' && (
                            <div className="h-full flex flex-col border border-gray-200 rounded">
                                ...
                                <div className="p-3 border-t border-gray-200 bg-gray-50 flex justify-end gap-2">
                                    <button onClick={() => { setActiveTab('write'); setReceiver(selectedNote.sender); setWriteContent(`[답장]\n\n-----------------\n${selectedNote.content}\n-----------------\n`); setSelectedNote(null); }} className="px-3 py-1 bg-blue-600 text-white text-xs sm:text-sm rounded hover:bg-blue-700">답장</button>
                        ...
                        {!selectedNote && activeTab !== 'write' && (
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-t-2 border-b border-blue-600 text-gray-700 bg-gray-50 text-xs sm:text-sm">
                                ...
                                <tbody>
                                    {(activeTab === 'inbox' ? inbox : activeTab === 'unread' ? unread : sent).map(note => (
                                        <tr key={note.id} className="border-b border-gray-200 hover:bg-blue-50 cursor-pointer transition-colors" onClick={() => handleView(note)}>
                                            ...
                                            <td className={`py-3 text-center hidden sm:table-cell ${note.isAdmin ? 'text-blue-600 font-bold' : 'text-gray-600'}`}>{note.sender}</td>
