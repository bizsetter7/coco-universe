...
export default function HomePortal() {
  ...
  const [user, setUser] = useState<{ type: 'shop' | 'user'; name: string } | null>(null);
  ...
  const baseFilteredShops = useMemo(() => {
    ...
    if (searchKeyword) {
      ...
      list = list.filter(shop => shop.name.toLowerCase().includes(keyword) || shop.region.toLowerCase().includes(keyword));
    }

    return list;
  }, [selectedRegion, selectedSubRegion, selectedCategory, selectedJobType, searchKeyword, shops]);
  ...
  // 리스트용 (Common/Native + Sorting)
  const filteredShops = useMemo(() => {
    return baseFilteredShops
      .filter(shop => shop.tier === 'common' || shop.tier === 'native' || shop.tier === 'basic' || !shop.tier)
      .sort((a, b) => {
  ...
  // Home Section Limits
  const [grandLimitHome, setGrandLimitHome] = useState(8);
  const [deluxeLimitHome, setDeluxeLimitHome] = useState(8);
  const [specialLimitHome, setSpecialLimitHome] = useState(8);
  const [urgentLimitHome, setUrgentLimitHome] = useState(8);

  const primaryStyle = { color: brand.primaryColor };
  const primaryBgStyle = { backgroundColor: brand.primaryColor };

  if (!isMounted) {
    return <div className={`w-full min-h-[100dvh] ${brand.theme === 'dark' ? 'bg-gray-900 border-gray-800' : 'bg-white'}`} />;
  }

  return (
    <div className={`w-full max-w-full min-h-[100dvh] relative pb-20 overflow-x-hidden ${brand.theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-white text-black'}`}>
      {/* Header */}
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {user ? (
              <div className="flex items-center gap-2">
                <span className={`text-xs sm:text-sm font-bold ${brand.theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>{user.name} 님</span>
                <button className="text-[10px] sm:text-xs text-gray-400 hover:text-red-500" onClick={() => setUser(null)}>로그아웃</button>
              </div>
            ) : (
      ...
      <main className="w-full">
        ...
        {currentPage === 'home' && (
          <div className="page-home">
            ...
            <div className="max-w-[1020px] mx-auto px-3 py-6 sm:py-8">
              ...
              {/* [RESTORE] 1. 그랜드 / 프리미엄 (Synchronized with JobClient) */}
              <div className="mb-10">
                <div className="flex items-center justify-between mb-4">
                  <h3 className={`flex items-center gap-2 text-xl font-black ${brand.theme === 'dark' ? 'text-white' : 'text-black'}`}>
                    <Star className="text-amber-500" size={22} />
                    그랜드/프리미엄
                  </h3>
                  <button
                    onClick={() => setCurrentPage('payment')}
                    className="px-4 py-2 rounded-xl text-xs font-bold bg-pink-600 text-white hover:bg-pink-700 transition"
                  >
                    광고신청
                  </button>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {shops.filter(s => s.tier === 'grand' || s.tier === 'premium').slice(0, 8).map((shop, idx) => {
                    ...
                    return (
                      <div
                        ...
                        className={`rounded-[22px] overflow-hidden border shadow-sm cursor-pointer hover:shadow-lg hover:-translate-y-1 transition-all ${brand.theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}
                      >
                        <div className={`h-24 md:h-28 ${tier.bg} relative flex items-center justify-center`}>
                          <span className={`absolute top-3 left-3 px-2 py-0.5 rounded text-[10px] font-black ${tier.bg} ${tier.text} shadow-sm border border-white/20`}>
                            {tier.label}
                          </span>
                          <Crown className="text-white/50" size={32} />
                          <span className="absolute bottom-3 right-3 bg-black/50 text-white text-[10px] px-2 py-0.5 rounded font-bold">
                            TOP {rank}
                          </span>
                        </div>
                        <div className="p-4">
                          <h4 className={`text-[14px] font-black truncate mb-1 ${brand.theme === 'dark' ? 'text-white' : 'text-black'}`}>
                            {shop.realName || shop.name}
                          </h4>
                          <p className="text-[12px] text-gray-500 truncate mb-2">{shop.region}</p>
                          <p className="text-[13px] font-black text-pink-600 truncate">{shop.pay || '급여 협의'}</p>
                        </div>
                      </div>
                    );
                  })}
              ...
              {/* [RESTORE] 2. 디럭스 (Synchronized with JobClient) */}
              <div className="mb-10">
                <div className="flex items-center justify-between mb-4">
                  <h3 className={`flex items-center gap-2 text-xl font-black ${brand.theme === 'dark' ? 'text-white' : 'text-black'}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {shops.filter(s => s.tier === 'deluxe').slice(0, 8).map((shop, idx) => (
                    <div
                      key={shop.id || idx}
                      onClick={() => setSelectedShop(shop)}
              ...
              {/* [RESTORE] 3. 스페셜 채용 정보 (Synchronized with JobClient) */}
              <div className="mb-14">
                <div className="flex items-center justify-between mb-6">
                  <h3 className={`text-xl font-black flex items-center gap-2 ${brand.theme === 'dark' ? 'text-white' : 'text-black'}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                  {shops.filter(s => s.tier === 'special').slice(0, 12).map((shop, idx) => (
                    <div
                      key={shop.id || idx}
                      onClick={() => setSelectedShop(shop)}
                      className={`rounded-xl overflow-hidden border shadow-sm cursor-pointer hover:shadow-lg hover:-translate-y-1 transition-all group ${brand.theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}
                    >
                      <div className="h-20 bg-gradient-to-r from-purple-600 to-pink-400 relative flex items-center justify-center">
                        <span className="absolute top-2 left-2 px-2 py-0.5 rounded text-[9px] font-black bg-purple-800 text-white shadow-sm border border-white/20">
                          스페셜
                        </span>
                        <Sparkles className="text-white/50" size={24} />
                        <div className="absolute top-1 right-1 bg-red-600 text-white text-[7px] px-1 py-0.5 rounded-sm font-black italic shadow-sm animate-pulse">HOT</div>
                      </div>
                      <div className="p-2.5">
                        <h4 className={`text-[12px] font-black truncate mb-1 ${brand.theme === 'dark' ? 'text-white' : 'text-black'}`}>
                          {shop.realName || shop.name}
                        </h4>
                        <p className="text-[11px] font-bold text-pink-500 truncate">{shop.pay.split(' ')[1] || shop.pay}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* [RESTORE] 4. 급구 및 추천채용 (Synchronized with JobClient) */}
              <div className="mb-16">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {shops.filter(s => s.tier === 'urgent' || s.tier === 'recommended').slice(0, 9).map((shop, idx) => {
                    const isUrgent = shop.tier === 'urgent';
                    return (
                      <div
                        key={shop.id || idx}
                        onClick={() => setSelectedShop(shop)}
                        className={`border rounded-2xl p-4 sm:p-5 hover:border-rose-400 shadow-sm hover:shadow-md transition-all cursor-pointer group flex gap-4 ${brand.theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-100'}`}
                      >
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xs font-extrabold shrink-0 overflow-hidden shadow-inner ${brand.theme === 'dark' ? 'bg-gray-900 border border-gray-700 text-blue-400' : 'bg-white border border-gray-100 text-blue-400'}`}>
                          {shop.name.substring(0, 1)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start mb-1">
                            <h5 className={`font-black text-[14px] truncate group-hover:text-rose-500 transition-colors ${brand.theme === 'dark' ? 'text-white' : 'text-black'}`}>{shop.realName || shop.name}</h5>
                            <span className={`text-[9px] font-black px-1.5 py-0.5 rounded shadow-sm ${isUrgent ? 'bg-red-500 text-white' : 'bg-green-500 text-white'}`}>
                              {isUrgent ? '급구' : '추천'}
                            </span>
                          </div>
              ...
              <div id="job-list-section" className="mt-8">
                <div className="flex items-center justify-between mb-6">
                  <h2 className={`text-xl md:text-2xl font-black flex items-center gap-2 ${brand.theme === 'dark' ? 'text-white' : 'text-gray-900'} `}>
                    <Briefcase size={24} className="text-pink-500" />
                    <span>실시간 채용 정보</span>
                  ...
                  <div className="flex items-center gap-3">
                    <Link href="/favorites" className="hidden sm:flex items-center gap-1.5 text-xs font-bold text-amber-500 hover:underline">
                      <Star size={14} fill="currentColor" />
                      내 보관함
                    </Link>
                    <button
                      onClick={() => setCurrentPage('payment')}
                      className="px-4 py-2 rounded-xl text-xs font-bold bg-pink-600 text-white hover:bg-pink-700 transition shadow-md active:scale-95"
                    >
                      광고신청
                    </button>
                  </div>
                ...
                {/* 필터 탭 (간소화 - 일반 및 네이티브 위주) */}
                <div className="flex gap-2 mb-6 overflow-x-auto pb-2 no-scrollbar">
                  {['전체', '일반', '네이티브'].map((tab) => (
                    <button
                      key={tab}
                      className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all border ${(tab === '전체')
                  ...
                  <div className="hidden md:block overflow-x-hidden min-w-0">
                    <table className="w-full text-left text-sm border-collapse table-fixed">
                      ...
                      <tbody className={`divide-y ${brand.theme === 'dark' ? 'divide-gray-800' : 'divide-gray-50'}`}>
                        {filteredShops.filter(s => !['grand', 'premium', 'special', 'urgent', 'recommended'].includes(s.tier || '')).length > 0 ? (
                          filteredShops
                            .filter(s => !['grand', 'premium', 'special', 'urgent', 'recommended'].includes(s.tier || ''))
                                  ...
                                  <td className="px-3 py-4 overflow-hidden">
                                    <div className="flex items-center gap-1.5 min-w-0">
                                      ...
                                      <span className={`truncate font-bold ${brand.theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                                        {shop.title || `${shop.name}에서 함께 일할 가족을 모집합니다.`}
                                      </span>
                                    </div>
                                  </td>
                          ...
                          <tr><td colSpan={6} className="py-20 text-center text-gray-400">등록된 최신 공고가 없습니다.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
                          ...
                          .map((shop, i) => {
                            ...
                            return (
                              <div
                                key={shop.id || i}
                                onClick={() => setSelectedShop(shop)}
                                className={`p-4 active:bg-gray-50 transition-colors flex justify-between items-start gap-3 ${brand.theme === 'dark' ? 'bg-gray-950 active:bg-gray-800' : 'bg-white'}`}
                              >
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-1.5 text-[12px] flex-wrap">
                                    <span className="text-blue-500 font-extrabold truncate flex items-center gap-0.5">
                                      ♥{shop.name}♥
                                    </span>
                                    <span className="text-gray-300">|</span>
                                    <span className="text-amber-700 font-bold">{shop.region.split(' ').slice(0, 2).join(' ')}</span>
                                    <span className="text-gray-300">|</span>
                                    <span className="font-black text-red-600">{shop.pay}</span>
                                  </div>
                                </div>
                                <button
                ...
                {visibleCount < filteredShops.length && (
                  <button
                    ...
                    className={`w-full mt-8 py-5 rounded-2xl border-2 border-dashed font-black text-sm transition-all active:scale-[0.98] ${brand.theme === 'dark'
                      ? 'border-gray-700 text-gray-500 hover:bg-gray-800/50'
                      : 'border-gray-200 text-gray-400 hover:bg-gray-50 hover:border-gray-300'
                      }`}
                  >
                    채용정보 더보기 ({filteredShops.length - visibleCount}개 남음)
                  </button>
                )}
              </div>
                ...
                <button
                  ...
                  onClick={() => {
                    if (loginId === 'admin_shop' && loginPassword === 'password123') {
                      setUser({ type: 'shop', name: '업주 관리자' });
                      setCurrentPage('home');
                    } else if (loginId === 'admin_user' && loginPassword === 'password123') {
                      setUser({ type: 'user', name: '일반 관리자' });
                      setCurrentPage('home');
                    } else {
        ...
        {
          currentPage === 'region' && (
            <div className="w-full max-w-[1020px] mx-auto py-0 md:py-8 min-h-[100dvh] bg-gray-50 md:bg-white overflow-x-hidden px-0 md:px-4">
              ...
              <div className="flex flex-col lg:flex-row gap-0 lg:gap-6">
                ...
                {/* 중앙 콘텐츠 영역 (지역 페이지) */}
                <div className="flex-1 px-4 pt-6 pb-20 md:px-0">
                  <div className="flex flex-col gap-4 mb-6">
                    <h3 className={`text-2xl md:text-3xl font-black flex items-center gap-2 ${brand.theme === 'dark' ? 'text-white' : 'text-black'} `}>
                      <span className="text-pink-600">|</span> {selectedRegion === '전체' ? '전국' : selectedRegion} 채용정보
                    </h3>
                    ...
                    >
                      <div className="flex items-center gap-2 overflow-hidden">
                        <span className="bg-pink-600 text-white text-[10px] font-black px-2 py-0.5 rounded-md whitespace-nowrap">공지</span>
                        <span className={`text-[12px] md:text-sm font-black truncate ${brand.theme === 'dark' ? 'text-gray-100' : 'text-black'} `}>
                          [필독] 안전한 거래를 위한 업소 인증 절차 안내
                        </span>
                      </div>
                    ...
                    <div className="flex border-b-2 border-gray-100 mt-2">
                      {['지역별 채용', '오늘본광고'].map((tab) => (
                        <button
                          key={tab}
                          onClick={() => setActiveRegionTab(tab === '지역별 채용' ? 'region' : 'other')}
                          className={`flex-1 py-3 text-[13px] md:text-sm font-black text-center relative transition-colors ${tab === '지역별 채용' ? 'text-pink-600 border-b-2 border-pink-600 -mb-0.5' : 'text-gray-400'
                            }`}
                        >
                          {tab}
                        </button>
                  ...
                  {/* 검색 폼 (상단 고정 스타일) */}
                  <div className={`p-4 rounded-2xl border shadow-sm space-y-3 mb-8 ${brand.theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
                    <div className="grid grid-cols-2 gap-2">
                      <select
                        className={`w-full p-3 rounded-xl text-xs font-bold border ${brand.theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-100'}`}
                        value={selectedRegion}
                        onChange={(e) => { setSelectedRegion(e.target.value); setSelectedSubRegion('전체'); }}
                      >
                        <option value="전체">지역전체</option>
                        {REGION_LIST.map(reg => <option key={reg} value={reg}>{reg}</option>)}
                      </select>
                      <select
                        disabled={selectedRegion === '전체'}
                        className={`w-full p-3 rounded-xl text-xs font-bold border disabled:opacity-50 ${brand.theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-100'}`}
                        value={selectedSubRegion}
                        onChange={(e) => setSelectedSubRegion(e.target.value)}
                      >
                        <option value="전체">세부지역</option>
                        {selectedRegion !== '전체' && (REGIONS_MAP[selectedRegion] as string[])?.map(sub => <option key={sub} value={sub}>{sub}</option>)}
                      </select>
                    </div>
                    <div className="flex gap-2">
                      <div className="relative flex-1">
                        <input
                          ...
                          placeholder="가게명 또는 키워드"
                          className={`w-full p-3 pl-10 rounded-xl text-xs font-bold border ${brand.theme === 'dark' ? 'bg-gray-700 border-gray-600 text-white' : 'bg-gray-50 border-gray-100'}`}
                        />
                        <Search size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
                      </div>
                      <button className="px-6 py-3 bg-gray-900 text-white rounded-xl text-xs font-black shadow-md active:scale-95">검색</button>
                    </div>
                  </div>

                  {/* 지역 특화 리스트 (홈과 동일한 카드 구조 사용) */}
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm font-black text-gray-400 uppercase tracking-wider">Filtered Listings</h4>
                      <span className="text-[10px] font-bold text-gray-400">Total {filteredShops.length} cases</span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {filteredShops.length > 0 ? (
                        filteredShops.slice(0, visibleCount).map((shop, i) => {
                          const isFav = favorites.includes(shop.id);
                          const isPremium = ['grand', 'premium', 'deluxe', 'special'].includes(shop.tier || '');

                          return (
                            <div
                              key={shop.id || i}
                              onClick={() => setSelectedShop(shop)}
                              className={`relative p-5 rounded-2xl border transition-all cursor-pointer group shadow-sm hover:shadow-md ${isPremium
                                ? (brand.theme === 'dark' ? 'bg-gray-800/50 border-pink-900/30' : 'bg-pink-50/20 border-pink-100')
                                : (brand.theme === 'dark' ? 'bg-gray-800/30 border-gray-700' : 'bg-white border-gray-100')
                                }`}
                            >
                              <div className="flex justify-between items-start mb-3">
                                <span className={`text-[10px] font-black px-2 py-0.5 rounded-lg border ${shop.tier === 'grand' ? 'bg-amber-500 text-white border-amber-600' :
                                  shop.tier === 'premium' ? 'bg-purple-600 text-white border-purple-700' :
                                    'bg-gray-500 text-white border-gray-600'
                                  }`}>
                                  {shop.tier?.toUpperCase() || 'NORMAL'}
                                </span>
                                <button onClick={(e) => toggleFavorite(e, shop.id)} className={`${isFav ? 'text-amber-400' : 'text-gray-300'}`}>
                                  <Star size={18} fill={isFav ? "currentColor" : "none"} />
                                </button>
                              </div>
                              <h4 className={`text-sm font-black mb-1 group-hover:text-pink-600 ${brand.theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{shop.name}</h4>
                              <p className="text-xs text-gray-400 mb-4 line-clamp-1">{shop.title || shop.name}</p>
                              <div className="flex items-center justify-between text-[11px] font-bold">
                                <span className="text-gray-400">{shop.region.split(' ').slice(0, 2).join(' ')}</span>
                                <span className="text-pink-600">{shop.pay}</span>
                              </div>
                            </div>
                          );
                        })
                      ) : (
                        <div className="col-span-full py-20 text-center text-gray-400 text-sm font-bold">검색 결과가 없습니다.</div>
                      )}
                    </div>
                    ...
                    {visibleCount < filteredShops.length && (
                      <button
                        onClick={() => setVisibleCount(prev => prev + 10)}
                        className={`w-full py-4 rounded-xl border-2 border-dashed font-bold text-xs ${brand.theme === 'dark' ? 'border-gray-700 text-gray-500' : 'border-gray-200 text-gray-400'}`}
                      >
                        지역 공고 더 읽어오기 +
                      </button>
                    )}
      ...
      <nav className={`md:hidden fixed bottom-0 left-0 right-0 w-full border-t flex justify-around py-3 z-40 text-[10px] text-gray-400 ${brand.theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200 shadow-[0_-2px_8px_rgba(0,0,0,0.05)]'}`}>
        ...
        <button onClick={() => router.push('/lounge')} className="flex flex-col items-center gap-1 hover:text-brand-primary transition-colors">
