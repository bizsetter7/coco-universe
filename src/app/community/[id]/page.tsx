...
function extractSeoKeywords(title: string, content: string, category: string): string {
    const BASE_KEYWORDS = [
        '코코알바', '코코판', '여성알바', '고소득알바', '룸알바', '언니들수다',
        '연애고민', '여자커뮤니티', '썰', '공감', '연애썰', '직장인고민'
    ];

    const CATEGORY_KEYWORDS: Record<string, string[]> = {
        '언니들의 수다(썰)': ['연애썰', '연애고민', '남친고민', '여자공감', '공감썰', '연애이야기'],
        '프리미엄 라운지': ['프리미엄정보', '재테크', '성공노하우', '재무관리', '고소득'],
        '밤 문화 Talk': ['밤알바', '유흥정보', '밤문화', '알바후기'],
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
    ...
    const description = `${post.content.slice(0, 120)}... 코코알바 커뮤니티에서 더 많은 이야기를 확인하세요.`;
