 ...
 *
 * STEP 1 — 인증 시작
 *   브라우저에서: https://www.cocoalba.kr/api/admin/band-auth?action=start
 *   → Band 로그인 페이지로 이동
 *   → 로그인 + 권한 허용
 ...
 *
 * STEP 3 — 밴드 키 확인
 *   https://www.cocoalba.kr/api/admin/band-auth?action=bands&token=ACCESS_TOKEN
...
const REDIRECT_URI = `${process.env.NEXT_PUBLIC_SITE_URL ?? 'https://www.cocoalba.kr'}/api/admin/band-auth?action=callback`;
