...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (data.user?.id) {
            ...
            // username: SignupPage에서 명시적으로 전달받은 아이디 사용. 없으면 email 앞부분 fallback
            ...
            if (birthdate) profilePayload.birth_date = birthdate; // [Fix] signup route에서 birth_date 미저장 버그 수정
            ...
            if (identity_ci) {
                ...
                profilePayload.is_adult_verified = true; // [Fix] 본인인증 완료 시 is_adult_verified 명시 저장
            }
