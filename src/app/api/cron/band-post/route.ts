 ...
 *
 * cron-job.org 설정:
 *   URL: https://www.cocoalba.kr/api/cron/band-post?secret=CRON_SECRET값
 *   주기: 6시간마다 (하루 4회)
 *
 * 필요 환경변수:
...
export async function GET(request: NextRequest) {
    ...
    const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.cocoalba.kr';
    ...
    try {
        const region   = (shop.region || '').replace(/[\[\]]/g, '').trim();
        const filename = `${region}-${shop.nickname || shop.name || 'cocoalba'}-구인공고.png`
