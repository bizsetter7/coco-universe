...
export default function SystemVerificationPage() {
    ...
    const checkTitleIssue = (title: string, name: string) => {
        const fullTitle = title || name;
        // PC 1-line loose alignment: approx 26-30 chars depending on Hangul/English
        // Mobile 1-line strict: approx 18-20 chars
        // We set a 'Warning zone' at 26 chars because we enforced maxLength=26 in form.
        return fullTitle.length > 26;
    };
        ...
        <div className="p-6 max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-6">
                <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
                    ...
                    시스템 검증 센터 (System Verification)
                </h1>
                <button
                    onClick={fetchShops}
                    className="flex items-center gap-2 px-4 py-2 bg-gray-100 rounded-xl hover:bg-gray-200 transition font-bold text-sm"
                >
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
                ...
                <div className="flex gap-2 mb-4">
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg font-bold text-sm transition ${filter === 'issue' ? 'bg-red-600 text-white shadow-md' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                    >
                        위반 의심 항목만 보기
                    </button>
                    <button
                        onClick={() => setFilter('all')}
                        className={`px-4 py-2 rounded-lg font-bold text-sm transition ${filter === 'all' ? 'bg-gray-800 text-white shadow-md' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                    >
                        전체 보기
                    </button>
                </div>
                                ...
                                filteredShops.map(shop => {
                                    ...
                                    return (
                                        <tr key={shop.id} className={`hover:bg-gray-50 transition ${isIssue ? 'bg-red-50/30' : ''}`}>
                                            <td className="p-3">
                                                {isIssue ? (
                                                    <span className="inline-flex items-center gap-1 px-2 py-1 bg-red-100 text-red-700 text-[10px] font-black rounded-full">
                                                        <AlertTriangle size={12} /> 위반
                                                    </span>
                                                ) : (
                                                    <span className="inline-flex items-center gap-1 px-2 py-1 bg-green-100 text-green-700 text-[10px] font-black rounded-full">
                                            ...
                                            <td className="p-3">
                                                <div className={`font-bold text-sm mb-0.5 line-clamp-1 break-all ${isIssue ? 'text-red-600' : 'text-gray-700'}`}>
                                                    {title}
                                                </div>
                                                <div className={`text-[10px] font-mono font-bold ${isIssue ? 'text-red-500' : 'text-gray-400'}`}>
                                                    Length: {title.length} / 26
                                                </div>
                                            </td>
                                            <td className="p-3">
                                                <a
                                                    ...
                                                    target="_blank"
                                                    rel="noreferrer"
                                                    className="inline-flex items-center gap-1 px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-bold text-gray-600 hover:bg-gray-50 hover:text-pink-600 transition"
                                                >
